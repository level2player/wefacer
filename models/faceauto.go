...
func (faceAddDentifyFace FaceAddDentifyFace) DentifyFace(request IRequest, requesthead RequestHead, responseContentChan chan string, faceAutoerrchan chan bool) {
	...
	res, err := http.PostForm(core.WefacerConfig.ConfigMap["faceadd_recognition_url"], url.Values{"api_key": {core.WefacerConfig.ConfigMap["faceadd_api_key"]}, "api_secret": {core.WefacerConfig.ConfigMap["faceadd_secret_secret_key"]}, "image_base64": {basestring}, "return_landmark": {"1"}, "return_attributes": {"gender,beauty,age,smiling,headpose,facequality,blur,eyestatus,emotion,ethnicity"}})
	if err != nil {
		log.Println(err.Error())
	}
	defer res.Body.Close()
...
var (
	FaceAutoStruct map[string]interface{}
)
