		...
		Attributes struct {
			...
			Eyestatus struct {
				LeftEyeStatus struct {
					...
					NoGlassEyeClose     int     `json:"no_glass_eye_close"`
					Occlusion           float64 `json:"occlusion"`
					NoGlassEyeOpen      float64 `json:"no_glass_eye_open"`
					NormalGlassEyeClose int     `json:"normal_glass_eye_close"`
					DarkGlasses         int     `json:"dark_glasses"`
...
type IdentifyFace interface {
	DentifyFace(request IRequest, requesthead RequestHead, requesthandlechan chan DefaultFaceAuto, faceAutoerrchan chan bool)
}
...
func (baiduDentifyFace BaiduDentifyFace) DentifyFace(request IRequest, requesthead RequestHead, requesthandlechan chan DefaultFaceAuto, faceAutoerrchan chan bool) {
	...
	defaultFaceAuto := DefaultFaceAuto{}
	defaultFaceAuto.FaceCount = baiduAuto.ResultNum
	for _, result := range baiduAuto.Result {
		tmpface := Face{int(result.Age), result.Race, result.Gender, result.Expression, result.Glasses}
		defaultFaceAuto.Face = append(defaultFaceAuto.Face, tmpface)
	}
	requesthandlechan <- defaultFaceAuto
}
...
func (faceAddDentifyFace FaceAddDentifyFace) DentifyFace(request IRequest, requesthead RequestHead, requesthandlechan chan DefaultFaceAuto, faceAutoerrchan chan bool) {
	imageRequest, ok := request.(ImageRequest)
	if !ok {
		faceAutoerrchan <- true
		return
	...
	res, err := http.PostForm(core.WefacerConfig.ConfigMap["faceadd_recognition_url"], url.Values{"api_key": {FaceAddKeyValue.APIKey}, "api_secret": {FaceAddKeyValue.APISecret}, "image_base64": {basestring}, "return_landmark": {"1"}, "return_attributes": {"gender,age,smiling,headpose,facequality,blur,eyestatus,emotion,ethnicity"}})
	if err != nil {
		log.Println(err.Error())
	}
	defer res.Body.Close()
	...
	defaultFaceAuto := DefaultFaceAuto{}
	defaultFaceAuto.FaceCount = len(faceAddAutoGenerated.Faces)
	for _, result := range faceAddAutoGenerated.Faces {
		tmpface := Face{int(result.Attributes.Age.Value), result.Attributes.Ethnicity.Value, result.Attributes.Gender.Value, 1, 1}
		defaultFaceAuto.Face = append(defaultFaceAuto.Face, tmpface)
	}
	requesthandlechan <- defaultFaceAuto
}
